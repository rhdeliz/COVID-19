...
if(RUN_FOR == "Texas") {
  DIRECTORY = "/Users/u_deliz/Downloads/COVID-19/Texas/"
  TABLE = "Texas COVID-19 Case Count Data by County.xlsx"
}

if(RUN_FOR == "US Counties") {
  DIRECTORY = "/Users/u_deliz/Downloads/COVID-19/USA"
...
if(RUN_FOR == "US Counties") {
  ...
  # b <- unique(USCountiesPopulation$Location)
  # a <- unique(Cases$Location)
  # 
  # ALL_COUNTIES <- b
  # MISSING <- match(b, a)
  # 
  # MISSING_COUNTIES <- cbind(ALL_COUNTIES, MISSING)
  # MISSING_COUNTIES <- as.data.frame(MISSING_COUNTIES)
  # MISSING_COUNTIES <-
  #   MISSING_COUNTIES %>%
  #   mutate(
  ...
  tryCatch({
    ...
    WaveFx <- function(WaveX){
      tryCatch({
        ...
        PredictionCases$Cases <- PredictionCases$Cases
        PredictionCases$CasesMin <- pm2[,2]
        PredictionCases$CasesMin <- PredictionCases$CasesMin
        PredictionCases$CasesMax <- pm2[,3]
        PredictionCases$CasesMax <- PredictionCases$CasesMax
        PredictionCases$Day <- PredictionCases$conc-15
